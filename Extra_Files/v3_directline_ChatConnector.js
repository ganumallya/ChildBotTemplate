...
var ChatConnector = (function () {
    ...
    ChatConnector.prototype.startConversation = function (address, done) {
        if (address && address.user && address.bot && address.serviceUrl) {
            var options = {
                method: 'POST',
                url: urlJoin(address.serviceUrl, '/v3/directline/conversations'),
                body: {
                    bot: address.bot,
    ...
    ChatConnector.prototype.delete = function (address, done) {
        var path = '/v3/directline/conversations/' + encodeURIComponent(address.conversation.id) +
    ...
    ChatConnector.prototype.getConversations = function (serviceUrl, continuationToken, done) {
        var path = '/v3/directline/conversations';
    ...
    ChatConnector.prototype.deleteConversationMember = function (serviceUrl, conversationId, memberId, done) {
        var path = '/v3/directline/conversations/' + encodeURIComponent(conversationId) +
    ...
    ChatConnector.prototype.exportBotStateData = function (serviceUrl, channelId, continuationToken, done) {
        var path = '/v3/directline/botstate/' + channelId + '/exportBotStateData';
        if (continuationToken) {
            path += '?continuationToken=' + encodeURIComponent(continuationToken);
        }
        var options = {
            ...
            if (context.userId) {
                ...
                if (context.conversationId) {
                    list.push({
                        ...
                        url: root + '/directline/conversations/' + encodeURIComponent(context.conversationId) +
    ...
    ChatConnector.prototype.saveData = function (context, data, callback) {
        ...
        try {
            ...
            if (context.userId) {
                ...
                if (context.conversationId) {
                    var url = root + '/directline/conversations/' + encodeURIComponent(context.conversationId) +
                        '/users/' + encodeURIComponent(context.userId);
                    addWrite('privateConversationData', data.privateConversationData || {}, url);
                }
            }
            if (context.persistConversationData && context.conversationId) {
                addWrite('conversationData', data.conversationData || {}, root + '/directline/conversations/' + encodeURIComponent(context.conversationId));
            }
            async.each(list, function (entry, cb) {
    ...
    ChatConnector.prototype.postMessage = function (msg, lastMsg, cb, method) {
        ...
        var path = '/v3/directline/conversations/' + encodeURIComponent(address.conversation.id) + '/activities';
    ...
    ChatConnector.prototype.getStoragePath = function (address) {
        ...
        return path + '/v3/directline/botstate/' + encodeURIComponent(address.channelId);
    };
    ...
    ChatConnector.prototype.prepIncomingMessage = function (msg) {
        ...
        msg.text = msg.text || '';
        //console.log("message text is"+ " "+msg.text);
